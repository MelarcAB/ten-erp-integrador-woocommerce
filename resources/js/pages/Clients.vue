...
type ClienteRow = {
    ...
    ten_persona: boolean | number | null;
    ten_enviar_emails: boolean | number | null;
    ten_consentimiento_datos: boolean | number | null;
...
const perPage = ref<number>(Number(initial.value.per_page ?? props.clientes.per_page ?? 20));
...
function applyFilters() {
    ...
    router.get(location.pathname, buildParams(), {
        ...
        preserveState: true,
        preserveScroll: true,
        only: ['clientes', 'filters', 'filtros', 'counts'],
        onFinish: () => {
...
const totalAll = computed(() => {
    ...
    return values.length ? values.reduce((a, b) => a + (Number(b) || 0), 0) : props.clientes.total;
});

const fullName = (c: ClienteRow) => {
    const n = [c.nombre, c.apellidos].filter(Boolean).join(' ').trim();
    return n || c.nombre_fiscal || '—';
};
</script>
        ...
        >
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">Estado de sync y calidad de datos (email/NIF/teléfonos).</p>
                </div>
            ...
            <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    ...
                    <div class="w-full md:max-w-md">
                        ...
                        <div class="relative">
                            <input
                                ...
                                placeholder="Nombre, email, NIF, códigos, ids…"
                            />
                ...
                <div class="text-sm text-muted-foreground">
                    <span class="tabular-nums">{{ props.clientes.from ?? 0 }}</span>
                    -
                    <span class="tabular-nums">{{ props.clientes.to ?? 0 }}</span>
                    de
                    <span class="font-medium tabular-nums text-foreground">{{ props.clientes.total }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-background dark:border-sidebar-border">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-border">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        <tr v-for="c in props.clientes.data" :key="String(c.woocommerce_id ?? c.ten_id ?? c.email ?? Math.random())" class="transition hover:bg-accent/30">
                            <td class="px-4 py-3">
                                <div class="flex flex-col">
                                    <div class="flex items-center gap-2">
                                        <div class="font-medium text-foreground">{{ fullName(c) }}</div>
                                    ...
                                    <div class="mt-1 text-xs text-muted-foreground">
                                        ...
                                        <span>{{ c.nombre_fiscal || '—' }}</span>
                                        <span class="mx-2">·</span>
                                        <span class="font-medium">NIF:</span>
                                        <span>{{ c.nif || '—' }}</span>
                                    </div>
                                    <div v-if="c.last_error" class="mt-2 line-clamp-2 rounded-md border border-rose-200 bg-rose-50 px-2 py-1 text-xs text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-200">
                                        {{ c.last_error }}
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 align-top">
                                <div class="inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-xs font-medium ring-1" :class="statusMeta(c.sync_status).cls">
                            ...
                            <td class="px-4 py-3 align-top">
                                <div class="text-sm">
                                    ...
                                    <div class="mt-1 flex items-center justify-between gap-3">
                                        <span class="text-xs text-muted-foreground">Tlf</span>
                                        <span class="text-xs">{{ c.telefono ?? '—' }}</span>
                                    </div>
                                    <div class="mt-1 flex items-center justify-between gap-3">
                                        <span class="text-xs text-muted-foreground">Tlf2</span>
                                        <span class="text-xs">{{ c.telefono2 ?? '—' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 align-top">
                                <div class="text-sm">
                                    <div class="flex items-center justify-between gap-3">
                                        ...
                                        <span class="font-mono text-xs tabular-nums">{{ c.ten_id ?? '—' }}</span>
                                    </div>
                                    <div class="mt-1 flex items-center justify-between gap-3">
                                        <span class="text-xs text-muted-foreground">WOO</span>
                                        <span class="font-mono text-xs tabular-nums">{{ c.woocommerce_id ?? '—' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 align-top">
                                <div class="flex flex-col gap-1 text-xs">
                                    <div class="flex items-center justify-between gap-3">
                                        ...
                                        <span class="font-medium" :class="toBool(c.ten_persona) ? 'text-emerald-700 dark:text-emerald-200' : 'text-gray-600 dark:text-gray-300'">
                                            {{ toBool(c.ten_persona) ? 'Sí' : 'No' }}
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-between gap-3">
                                        <span class="text-muted-foreground">Emails</span>
                                        <span class="font-medium" :class="toBool(c.ten_enviar_emails) ? 'text-emerald-700 dark:text-emerald-200' : 'text-gray-600 dark:text-gray-300'">
                                            {{ toBool(c.ten_enviar_emails) ? 'Sí' : 'No' }}
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-between gap-3">
                                        <span class="text-muted-foreground">Consentimiento</span>
                                        <span class="font-medium" :class="toBool(c.ten_consentimiento_datos) ? 'text-emerald-700 dark:text-emerald-200' : 'text-gray-600 dark:text-gray-300'">
                                            {{ toBool(c.ten_consentimiento_datos) ? 'Sí' : 'No' }}
                                        </span>
                                    </div>
                                </div>
                            </td>
            ...
            <div class="flex flex-col gap-2 border-t border-border bg-background px-4 py-3 md:flex-row md:items-center md:justify-between">
                <div class="text-xs text-muted-foreground">
                    Página <span class="font-medium tabular-nums text-foreground">{{ props.clientes.current_page }}</span>
                    de <span class="font-medium tabular-nums text-foreground">{{ props.clientes.last_page }}</span>
                </div>

                <nav class="flex flex-wrap items-center gap-1">
                    <button
                        v-for="(l, idx) in props.clientes.links"
                        :key="idx"
                        type="button"
