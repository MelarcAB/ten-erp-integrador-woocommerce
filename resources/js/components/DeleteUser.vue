    ...
    <div class="space-y-6">
        ...
        <div class="space-y-4 rounded-lg border border-red-100 bg-red-50 p-4 dark:border-red-200/10 dark:bg-red-700/10">
            <div class="relative space-y-0.5 text-red-600 dark:text-red-100">
                <p class="font-medium">{{ $t('settings.important') }}</p>
                <p class="text-sm">{{ $t ('settings.delete_account_warning') }}</p>
            </div>
            <Dialog>
                ...
                <DialogContent>
                    <form class="space-y-6" @submit="deleteUser">
                        <DialogHeader class="space-y-3">
                            <DialogTitle>{{ $t('settings.delete_account_confirmation') }}</DialogTitle>
                            <DialogDescription>
                              {{ $t('settings.delete_confirmation_description') }}
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        <DialogFooter class="gap-2">
                            <DialogClose as-child>
                                <Button variant="secondary" @click="closeModal"> {{ $t('settings.cancel') }} </Button>
                            </DialogClose>
